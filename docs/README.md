...
├── 2-web/    # 프론트엔드 설계: 화면, 컴포넌트, 상태, API 연동
└── 3-api/    # 백엔드 설계: 데이터 모델, API, 비즈니스 로직
```
...
- **파일명은 영문 kebab 슬러그.** 세 폴더에서 같은 슬러그를 쓰고 상호 링크로 연결한다.
- **한 파일 = 한 기능/한 주제.** 길어지면 분할한다.
- **상호 참조는 상대 경로로.** 복사 후 템플릿의 `<기능명>`을 실제 슬러그로 치환한다.
